...
const zhCN = {
  ...
  model: {
    ...
    ownSanitizeHint: '勾上会设置 LLM_SCHEMA_SANITIZE=1',
  },
  pull: {
    title: '正在拉取镜像',
    ...
    stateFailed: '失败',
    statePending: '等待',
    failHint: '拉取失败会自动重试 3 次；仍然失败可以在设置里换一个镜像源，或从离线包导入。',
    roles: {
      web: '界面',
      api: '数据与分析服务',
      opencode: '对话引擎',
  ...
  settings: {
    ...
    checkUpdate: '自动检查更新',
    registry: '镜像源',
    registryHint: '目前只有 ghcr.io/agentpit-io 一个可用源（M0 实测阿里云 ACR 与 Docker Hub 上都没有镜像）。可以手填自建仓库地址。',
    hunterTag: 'Hunter 版本',
    workDir: '工作目录',
    telemetry: '匿名遥测',
  ...
  error: {
    ...
    codes: {
      ...
      E_NOT_IMPLEMENTED: { title: '这一部分还没实现', hint: 'M1 只做界面骨架，真实调用在 M2 / M3 里补。' },
