...
const en: Dict = {
  ...
  booting: {
    title: 'Checking Hunter…',
    body: 'Looking at whether Hunter is already running on this computer, so we can show you the right page. This step only reads; it changes nothing.',
  },
  dataFound: {
    title: 'Found data from last time',
    intro:
      "Hunter's containers are gone from this computer, but your data is still here. Installing again will reuse it — no database rebuild, no new secrets, and you stay logged in.",
    summary: 'Verdict',
    volumes: 'What is still here',
    continue: 'Install again (reuse this data)',
    dig: 'Look inside',
    digging: 'Looking…（~15s）',
    noVolumes: 'No volumes found for this project',
    jwt: 'Login secret (JWT_SECRET)',
    jwtKept: 'still here — reused as-is, you stay logged in',
    jwtGone: 'gone — you will have to log in again',
    secrets: 'Secrets volume (decrypts your encrypted settings)',
    secretsGone: 'gone',
    pgVersion: 'Database version',
    backups: (n: number) => `${n} backup(s) in the backup folder`,
    deepLine: (tables: number | null, write: string | null, mig: string | null) =>
      [
        tables === null ? null : `${tables} tables`,
        write === null ? null : `last updated ${write}`,
        mig === null ? null : `migrated up to ${mig}`,
      ]
        .filter(Boolean)
        .join(' · ') || 'Could not read any more detail this time',
  },
  monitor: {
    host: 'This computer',
    hostSource: 'system API',
    cpu: 'CPU',
    cores: (n: number) => `${n} cores`,
    mem: 'Memory',
    pressure: 'Memory pressure',
    systemDisk: 'System disk',
    freeOf: (free: string, total: string) => `${free} free / ${total}`,
    runtime: 'Hunter runtime',
    runtimeSource: 'measured inside the VM',
    runtimeNa: 'This computer uses your own Docker, so there is no Hunter-only VM layer.',
    vmSpec: 'Allocated',
    vmSpecValue: (cpus: number, mem: string) => `${cpus} cores · ${mem}`,
    vmMem: 'Memory used',
    vmDisk: 'Disk used',
    data: 'Data',
    dataSource: 'measured via docker',
    db: 'Database',
    volumes: 'Volumes total',
    volumeCount: (n: number) => `(${n})`,
    images: 'Images',
    imagesFound: (n: number) => `(${n} of 6 found)`,
    services: 'Services',
    restartTip: (n: number, oom: boolean) =>
      oom ? `restarted ${n}x · was killed for running out of memory` : `restarted ${n}x`,
  },
  stackOp: {
    stopTitle: 'Stop Hunter',
    stopBody: 'The six services will stop. Containers, the database and all your data stay; you can start it again any time.',
    stopAlsoRuntime: (gb: number) => `Also stop the Hunter runtime (frees about ${gb} GB of memory)`,
    stopAlsoRuntimeHint:
      'Hunter runs inside a VM dedicated to it. Stopping only the services leaves that VM holding memory. Your data lives on the VM disk and is not lost when it stops.',
    stopConfirm: 'Stop',
    startTitle: 'Start Hunter',
    startBody: 'If the runtime is not running it will be started first, then we wait for the six services to become healthy.',
    restartOne: 'Restart one service',
    restartAll: 'Restart everything',
    pickService: 'Pick a service',
    working: 'Working…',
    steps: 'What just happened',
    doneIn: (ms: number) => `took ${(ms / 1000).toFixed(1)}s`,
  },
  steps: {
  ...
  dashboard: {
    ...
    envInstalledAt: 'Installed at',
    envInstalledAtNone: 'No install record yet',
    envLastHealthy: 'Last healthy',
    envLastHealthyNone: 'Never seen all six healthy yet',
    ...
    backupFailed: (why: string) => `Last backup failed: ${why}`,
    openBackup: 'Backup & restore',
    more: 'More',
    uninstall: 'Remove Hunter',
    alertsMore: (n: number) => `${n} more`,
    alertDetails: 'Details',
    alertFix: 'Free up space',
    alertAsk: 'Ask the AI',
    alertAsking: 'Asking…',
    alertDismiss: 'Got it',
    alertAdvice: 'Suggestion',
    alertCleanupDone: (s: string) => `Cleanup done: ${s}`,
  ...
  backup: {
    ...
    intro:
      'A backup holds the database, the secrets volume, your own skills and past conversations, plus the config file that contains your key. The whole folder is readable only by you (700) — do not share it.',
    dir: 'Backup folder',
    changeDir: 'Change folder',
    openDir: 'Open folder',
    diskFree: (s: string) => `${s} free on that disk`,
    count: (n: number, s: string) => `${n} backups · ${s} total`,
    create: 'Back up now',
    creating: 'Backing up…',
    ...
    fromOther: 'Pick a backup from another folder',
    fromOtherHint: 'Moving to a new computer: copy the whole backup folder over, then point the launcher at it here.',
    scheduleTitle: 'Scheduled backup',
    scheduleOn: (time: string) => `Every day at ${time}`,
    ...
    restoreBody:
      'Your current data will be replaced by this backup. The launcher backs up the current data first, so you can come back if you picked the wrong one.',
    restoreSteps: 'Restoring will: stop api / opencode / web → load the database back → restore the secrets volume and JWT_SECRET → start everything → wait for all six services to be healthy.',
    restoreType: (phrase: string) => `To confirm, type "${phrase}" exactly`,
    restoreGo: 'Restore',
    restoring: 'Restoring…',
    restoreBlocked: 'This backup cannot be restored right now',
    restoreDone: 'Restore finished',
    safetyBackup: (id: string) => `Your previous data was saved as ${id}`,
    checksumBad: 'Some files do not match what was recorded',
    needsUpgrade: 'This backup comes from a newer Hunter; upgrade first, then restore',
  },
  uninstall: {
    title: 'Remove Hunter',
    step1: 'Step 1: how much do you want removed',
    scopeAppOnly: 'Remove the app, keep the data',
    scopeAppOnlyHint: 'Containers and the launcher config are removed; the database, secrets volume, past conversations and your own skills all stay. Reinstalling later reuses them and you stay logged in.',
    scopeAll: 'Remove the app and all data',
    scopeAllHint: 'Everything above plus every data volume and the config. Files in the backup folder are not touched.',
    recommended: 'recommended',
    optImages: (s: string) => `Also remove the Hunter images (${s})`,
    optImagesHint: 'Reinstalling would download them again.',
    optRuntime: (s: string) => `Also remove the Hunter runtime (${s})`,
    optBackup: 'Make a backup first',
    optBackupHint: 'It goes to the backup folder, which removal never touches.',
    dataSummary: (tables: number, at: string) => `The database has ${tables} tables; last written at ${at}`,
    dataSummaryUnknown: 'Could not read the table count (the container may not be running)',
    noBackup: 'There is no backup on this machine yet',
    willFree: (s: string) => `Frees roughly ${s}`,
    keepBackupDir: (dir: string) => `The backup folder ${dir} is never removed`,
    step2: 'Step 2: confirm',
    typeHint: (phrase: string) => `Type "${phrase}" exactly`,
  ...
  cleanup: {
    title: 'Free up space',
    intro: "Only Hunter's own leftovers: images from other versions, volumes nothing uses, and expired backups. Nothing else on your machine is touched.",
    oldImages: (n: number, s: string) => `${n} old images · ${s}`,
    orphanVolumes: (n: number, s: string) => `${n} orphaned volumes · ${s}`,
    expiredBackups: (n: number, s: string) => `${n} expired backups · ${s}`,
    total: (s: string) => `${s} in total`,
    nothing: 'Nothing can be safely removed right now.',
    go: 'Clean up',
    working: 'Cleaning…',
  },
  settings: {
    ...
    backupDirHint: "Kept outside Hunter's working directory, so removing the app never touches it.",
    backupDirPick: 'Choose a folder',
    backupDirSuggest: (d: string) => `Suggested: ${d}`,
    backupDirExternal: (d: string) => `An external drive is safer: ${d}`,
    backupKeepDays: 'Keep the last N days',
    backupKeepDaysHint: 'One per day (the last one of that day). Pre-upgrade backups keep their own last 2 and do not use this quota.',
    backupIncludes: 'What goes in',
    backupIncludeDb: 'Database (required)',
    backupIncludeSecrets: 'Secrets volume (required, pairs with the database)',
    backupIncludeEnv: 'Config .env (required, contains your key)',
    backupIncludeSkills: 'Your own skills',
    backupIncludeSessions: 'Past conversations',
    backupSchedule: 'Scheduled task',
    backupLastOk: 'Last success',
    backupNever: 'never succeeded yet',
    ...
    monitorNotify: 'System notification for critical alerts',
    monitorNotifyHint: 'Only the critical tier pops up. Warnings stay on the dashboard.',
    monitorHostDiskWarn: 'Warn when system disk free is below (GB)',
    monitorHostDiskCrit: 'Critical below (GB)',
    monitorVmDiskWarn: 'Warn when runtime disk is above (%)',
    monitorVmMemWarn: 'Warn when runtime memory is above (%)',
    monitorAlerts: 'Active alerts',
    monitorNone: 'Nothing looks wrong',
