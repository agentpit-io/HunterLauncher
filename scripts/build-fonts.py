...
def collect_chars() -> str:
    ...
    Rust 源码里的**注释**也会被一起扫进来。这是有意的：想只扫字符串字面量就得解析 Rust
    （还要处理 `r#"…"#` 这种跨行的原始字符串），一旦漏判就是界面上一个方块。
    多扫几百个字形对 woff2 的体积影响很小（中文子集本来就以千计）。

    代价是**改注释也会触发重新生成** —— CI 的 `check-fonts.py` 会提醒你跑这个脚本。
    这个代价是划算的一边。
    """
    chars = set(BASE)
